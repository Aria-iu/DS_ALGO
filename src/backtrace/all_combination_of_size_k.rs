...
pub fn generate_all_combinations2(n: usize, k: usize) -> Result<Vec<Vec<usize>>, CombinationError> {
    if n == 0 && k > 0 {
        return Err(CombinationError::InvalidZeroRange);
    }
    ...
    backtrack2(0, n, k, &mut current, &mut combinations);
    Ok(combinations)
}
...
fn backtrack2(
    ...
    for num in start..=(n - k + len) {
        current.push(num);
        backtrack2(num + 1, n, k, current, combinations);
        current.pop();
    }
}
        ...
        ($($name:ident: $test_case:expr,)*) => {
            $(
                ...
                fn $name() {
                    ...
                    assert_eq!(generate_all_combinations2(n, k), expected);
                }
            )*
        }
    }
